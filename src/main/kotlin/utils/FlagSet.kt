...
fun Byte.toBooleanArray() = BooleanArray(Byte.SIZE_BITS) {
	(this.toInt() shr it) and 1 != 0
}

fun Short.toBooleanArray() = BooleanArray(Short.SIZE_BITS) {
	(this.toInt() shr it) and 1 != 0
}

fun Int.toBooleanArray() = BooleanArray(Int.SIZE_BITS) {
	(this shr it) and 1 != 0
}

fun Long.toBooleanArray() = BooleanArray(Long.SIZE_BITS) {
	(this shr it) and 1 != 0L
}


fun BooleanArray.toByte() = this.foldIndexed(0) { index, accumulator, value ->
	accumulator + (if (value) 1 shl index else 0)
}.toByte()

fun BooleanArray.toShort() = this.foldIndexed(0) { index, accumulator, value ->
	accumulator + (if (value) 1 shl index else 0)
}.toShort()

fun BooleanArray.toInt() = this.foldIndexed(0) { index, accumulator, value ->
	accumulator + (if (value) 1 shl index else 0)
}

fun BooleanArray.toLong() = this.foldIndexed(0L) { index, accumulator, value ->
